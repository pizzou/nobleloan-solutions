...
export default function SettingsPage() {
  ...
  const handleEmailChange = async (e: React.FormEvent) => {
    ...
    if (!email.trim()) e2.email = "Email is required";
      ...
      e2.email = "Enter a valid email address";
    if (email.trim().toLowerCase() === user?.email?.toLowerCase())
      e2.email = "This is already your current email";
  ...
  const handlePasswordChange = async (e: React.FormEvent) => {
    ...
    if (currentPw === newPw && currentPw)
      e2.newPw = "New password must differ from current";
    setPwErrors(e2);
    if (Object.keys(e2).length > 0) return;
        ...
        <form onSubmit={handleEmailChange} className="space-y-4">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-1">
              You&apos;ll use this new address to log in going forward.
            </p>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
            ...
            />
            {emailErrors.emailPw && (
              <p className="text-xs text-red-600 mt-1">{emailErrors.emailPw}</p>
            )}
          </div>
          <button
            type="submit"
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        ...
        <form onSubmit={handlePasswordChange} className="space-y-4">
          <div>
            ...
            />
            {pwErrors.currentPw && (
              <p className="text-xs text-red-600 mt-1">{pwErrors.currentPw}</p>
            )}
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
            ...
            />
            {pwErrors.newPw && (
              <p className="text-xs text-red-600 mt-1">{pwErrors.newPw}</p>
            )}
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
            ...
            />
            {pwErrors.confirmPw && (
              <p className="text-xs text-red-600 mt-1">{pwErrors.confirmPw}</p>
            )}
          </div>
          <div className="pt-1">
            <button
