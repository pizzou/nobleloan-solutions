...
const DEFAULT_TESTIMONIALS: Testimonial[] = [
  {
    ...
    text: "Great service and fast approval.",
    rating: 5,
  },
];
...
const inputCls =
  "w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500";
const smallInputCls =
  "w-full border border-gray-300 rounded-lg px-2.5 py-2 text-xs focus:outline-none focus:ring-2 focus:ring-teal-500";
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
      <div className="flex items-center justify-between border-b border-gray-100 pb-3">
        ...
        <button
          onClick={add}
          className="text-teal-600 text-sm font-semibold hover:underline whitespace-nowrap"
        >
          + Add
        </button>
      </div>
      <div className="space-y-3">
        {items.length === 0 && (
          <p className="text-sm text-gray-400 italic">
            Nothing added yet — click &quot;+ Add&quot; to create one.
          </p>
        )}
        {items.map((item, idx) => (
          <div
            key={idx}
            className="border border-gray-100 rounded-lg p-4 bg-gray-50/50 relative"
          >
            <button
              onClick={() => remove(idx)}
              className="absolute top-2 right-2 text-gray-300 hover:text-red-500 text-xs font-bold w-6 h-6 flex items-center justify-center"
            >
              ✕
            </button>
...
export default function WebsiteSettingsPage() {
  ...
  const set = (patch: Partial<OrgData>) =>
  ...
  return (
    <div className="space-y-6 max-w-3xl pb-10">
      <div className="flex items-center justify-between sticky top-0 bg-gray-50/95 backdrop-blur z-10 py-3 -mx-1 px-1">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Full control over your public site — home, services, about, and
            contact.
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <Field label="Founded Year">
            <input
              ...
              onChange={(e) =>
                set({ foundedYear: Number(e.target.value) || undefined })
              }
            />
          </Field>
        </div>
      ...
      <RepeaterSection
        ...
        hint="4 highlight numbers shown on your homepage"
        items={org.stats ?? []}
        onChange={(stats) => set({ stats })}
        newItem={{ icon: "📈", value: "", label: "" }}
        renderRow={(s, update) => (
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-1">
            Your real rates and limits now live on a dedicated page — they drive
            both your website and actual loan approvals.
          </p>
        </div>
        <a
      ...
      {/* Team */}
      <RepeaterSection
        title="About Page — Leadership Team"
        items={org.team ?? []}
        ...
        renderRow={(m, update) => (
          <div className="grid grid-cols-3 gap-2">
            ...
            <input
              ...
              value={m.initials}
              onChange={(e) =>
                update({ initials: e.target.value.toUpperCase().slice(0, 2) })
              }
      ...
      {/* Testimonials */}
      <RepeaterSection
        title="Home Page — Client Testimonials"
        items={org.testimonials ?? []}
          ...
          <div className="space-y-2">
            <div className="grid grid-cols-2 gap-2">
              ...
              <input
                ...
                value={t.role}
                onChange={(e) => update({ role: e.target.value })}
                placeholder="Role e.g. Small Business Owner"
              />
            </div>
            <textarea
              className={smallInputCls}
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-5">
        ...
        <Field
          ...
          hint="Optional — shown on your Contact page"
        >
          <input
            className={inputCls}
            value={org.mapUrl ?? ""}
