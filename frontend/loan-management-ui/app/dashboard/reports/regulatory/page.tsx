    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500">
          BNR portfolio reports and credit bureau data exports, with secure API
            ...
            {[
              {
                ...
                value: fmt(summary.totalPrincipalDisbursed, currency),
                color: "text-indigo-600",
              },
              {
                label: "Outstanding Principal",
                value: fmt(summary.outstandingPrincipal, currency),
                color: "text-blue-600",
              },
              {
                label: "Total Interest Collected",
                value: fmt(summary.totalInterestCollected, currency),
                color: "text-green-600",
              },
              {
                label: "Interest Accrued (Unpaid)",
                value: fmt(summary.interestAccruedUnpaid, currency),
                color: "text-orange-600",
              },
              {
                label: "Portfolio at Risk (PAR)",
                value: `${fmt(summary.parAmount, currency)} (${((summary.parRatio || 0) * 100).toFixed(1)}%)`,
                color: "text-amber-600",
              },
              {
                label: "NPL (>90 days)",
                value: `${fmt(summary.nplAmount, currency)} (${((summary.nplRatio || 0) * 100).toFixed(1)}%)`,
                color: "text-red-600",
              },
            ].map((c) => (
              <div
...
function CreditBureauTab({ isAdmin }: { isAdmin: boolean }) {
  ...
  const load = useCallback(() => {
    ...
    setLoadError("");
    regulatoryApi
      .creditBureauPreview({ from: from || undefined, to: to || undefined })
  ...
  return (
    <div className="space-y-4">
      <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-xs text-amber-800">
        This screen contains borrower-level personal data (national ID, phone,
        date of birth). Every view and export here is written to the audit log.
      </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-x-auto">
          ...
          {records.length > 200 && (
            <p className="text-center text-xs text-gray-400 py-3">
              Showing first 200 of {records.length} — full data is in the
              export.
            </p>
          )}
        </div>
...
function ApiKeysTab() {
  ...
  const [form, setForm] = useState({
    name: "",
    clientType: "BNR" as "BNR" | "CREDIT_BUREAU",
    contactEmail: "",
    description: "",
  });
  ...
  const load = () => {
    ...
    regulatoryApi
      .listApiClients()
      .then((r) => setClients(r as ApiClient[]))
      .catch((e) => {
        console.error(e);
        setLoadError(
          e?.response?.data?.error || e?.message || "Could not load API keys.",
        );
      })
      .finally(() => setLoading(false));
  };
  ...
  const create = async () => {
    ...
    setSaving(true);
    try {
      const res = (await regulatoryApi.createApiClient(form)) as {
        apiKey: string;
        client: ApiClient;
      };
      setNewKey(res);
  ...
  const revoke = async (id: number) => {
    if (
      ...
      return;
    try {
      await regulatoryApi.revokeApiClient(id);
      load();
    } catch (e) {
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-sm text-gray-500 max-w-2xl">
          Issue API keys for external regulatory systems. A BNR key can only
          call the BNR report endpoints; a Credit Bureau key can only call the
          credit bureau export endpoints. Keys are scoped to your organization.
        </p>
            ...
            <tbody>
              {clients.map((c) => (
                <tr key={c.id} className="border-t border-gray-50">
                  ...
                  <td className="px-4 py-2">
                    <span
                      className={`text-xs px-2 py-0.5 rounded-full ${c.clientType === "BNR" ? "bg-blue-100 text-blue-700" : "bg-purple-100 text-purple-700"}`}
                    >
                      {c.clientType === "BNR"
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setForm((f) => ({ ...f, clientType: e.target.value as any }))
              }
              className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm"
            >
        ...
        {newKey && (
          <div className="space-y-3">
            <p className="text-sm text-gray-600">
              ...
              Give it to{" "}
              {newKey.client.clientType === "BNR" ? "BNR" : "the credit bureau"}{" "}
              to use in the{" "}
