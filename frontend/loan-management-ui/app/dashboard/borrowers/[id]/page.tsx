...
import { BorrowerDetails, BorrowerLoanSummary, BorrowerPayment } from "@/types";
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const response = await borrowerApi.getDetails(borrowerId);
        ...
        <Card>
          <div className="p-10 text-center">
            ...
            <p className="mt-2 text-sm leading-6 text-slate-500">
              {error || "Borrower details could not be loaded."}
            </p>
      ...
      <div className="max-w-[1600px] mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            <Button
              variant="outline"
              onClick={() =>
                router.push(`/dashboard/borrowers/${borrowerId}/edit`)
              }
            >
              Edit Profile
            </Button>
                ...
                <div className="flex items-start gap-5">
                  ...
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      {details.status && <StatusBadge value={details.status} />}
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2 mt-4">
                      {details.goodPayer && (
                        <Badge text="Good Payer" type="success" />
                      )}

                      {details.currentlyOverdue && (
                        <Badge text="Currently Overdue" type="danger" />
                      )}

                      {details.hasDefaultHistory && (
                        <Badge text="Default History" type="warning" />
                      )}

                      {details.hasMultipleActiveLoans && (
                        <Badge text="Multiple Active Loans" type="warning" />
                      )}
                    </div>
                  </div>
                </div>
        ...
        <section>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-4">
            <MetricCard
              label="Total Loans"
              value={formatNumber(details.totalLoans)}
              icon="▣"
            />

            <MetricCard
              label="Active Loans"
              value={formatNumber(details.activeLoans)}
              icon="↗"
              tone="teal"
            />
            ...
            <MetricCard
              label="Outstanding"
              value={formatCurrency(details.totalOutstanding, currency, locale)}
              icon="◉"
              tone="amber"
            />
            ...
            <MetricCard
              label="Total Paid"
              value={formatCurrency(details.totalPaid, currency, locale)}
              icon="✓"
              tone="green"
            />
            ...
            <MetricCard
              label="Overdue Loans"
              value={formatNumber(details.overdueLoans)}
              icon="!"
              tone={details.overdueLoans > 0 ? "red" : "green"}
            />
          </div>
        </section>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          ...
          >
            <InfoGrid>
              ...
              <Info label="Alternate Phone" value={details.alternatePhone} />

              <Info label="National ID" value={details.nationalId} mono />

              <Info label="Passport" value={details.passportNumber} />

              <Info
                label="Date of Birth"
                value={
                  details.dateOfBirth
                    ? formatDate(details.dateOfBirth, locale)
                    : undefined
                }
              />
              ...
              <Info label="Marital Status" value={details.maritalStatus} />

              <Info
                label="Nationality"
                value={details.nationality || "Rwandan"}
              />

              <Info label="Place of Birth" value={details.placeOfBirth} />
          ...
          <ProfileCard
            title="Residential & CRB Address"
            subtitle="Regulatory identity and physical-address profile"
          >
            <InfoGrid>
              <Info
                label="Address Line 1"
                value={details.addressLine1 || details.address}
              />
              <Info label="Address Line 2" value={details.addressLine2} />
              <Info label="City" value={details.city} />
              <Info
                label="Province"
                value={details.physicalAddressProvince || details.stateProvince}
              />
              <Info label="District" value={details.physicalAddressDistrict} />
              <Info label="Sector" value={details.physicalAddressSector} />
              <Info label="Cell" value={details.physicalAddressCell} />
              <Info label="Village" value={details.physicalAddressVillage} />
              <Info label="Postal Code" value={details.postalCode} />
            </InfoGrid>
          </ProfileCard>
          ...
          >
            <InfoGrid>
              <Info label="Employer" value={details.employerName} />

              <Info label="Employment Type" value={details.employmentType} />

              <Info label="Job Title" value={details.jobTitle} />

              <Info
                label="Monthly Income"
                value={formatCurrency(details.monthlyIncome, currency, locale)}
              />
              ...
              <Info
                label="Net Worth"
                value={formatCurrency(details.netWorth, currency, locale)}
              />

              <Info label="Credit Bureau" value={details.creditBureau} />

              <Info
                label="Credit Report Date"
                value={
                  details.creditReportDate
                    ? formatDate(details.creditReportDate, locale)
                    : undefined
                }
              />
            </InfoGrid>
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <div className="mt-1 text-sm font-semibold text-slate-800">
                  {details.address || "No address recorded."}
                </div>
              </div>
            </div>
          </div>
        ...
        <section>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-6 gap-4">
            <SmallStat label="Completed" value={details.completedLoans} />

            <SmallStat
              label="Defaulted"
              value={details.defaultedLoans}
              danger={details.defaultedLoans > 0}
            />

            <SmallStat
              label="Written Off"
              value={details.writtenOffLoans}
              danger={details.writtenOffLoans > 0}
            />

            <SmallStat
              label="Total Borrowed"
              value={formatCurrency(details.totalBorrowed, currency, locale)}
            />

            <SmallStat
              label="Total Disbursed"
              value={formatCurrency(details.totalDisbursed, currency, locale)}
            />

            <SmallStat label="Total Payments" value={details.totalPayments} />
          </div>
        </section>
        ...
        >
          <div className="grid grid-cols-2 md:grid-cols-4 gap-y-7 gap-x-6">
            <PerformanceMetric
              ...
              value={`${Number(details.repaymentRate ?? 0).toFixed(1)}%`}
              progress={Number(details.repaymentRate ?? 0)}
            />

            <PerformanceMetric
              label="On-Time Rate"
              value={`${Number(details.onTimePaymentRate ?? 0).toFixed(1)}%`}
              progress={Number(details.onTimePaymentRate ?? 0)}
            />

            <PerformanceMetric
              label="Successful Payments"
              value={formatNumber(details.successfulPayments)}
            />

            <PerformanceMetric
              label="Missed Payments"
              value={formatNumber(details.missedPayments)}
              danger={details.missedPayments > 0}
            />

            <PerformanceMetric
              label="Overdue Payments"
              value={formatNumber(details.overduePayments)}
              danger={details.overduePayments > 0}
            />

            <PerformanceMetric
              label="Current Days Past Due"
              value={formatNumber(details.currentDaysPastDue)}
              danger={details.currentDaysPastDue > 0}
            />

            <PerformanceMetric
              label="Maximum Days Past Due"
              value={formatNumber(details.maximumDaysPastDue)}
              danger={details.maximumDaysPastDue > 0}
            />
        ...
        >
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <RiskBox
              label="Good Payer"
              value={details.goodPayer ? "Yes" : "No"}
              tone={details.goodPayer ? "good" : "bad"}
            />

            <RiskBox
              label="Currently Overdue"
              value={details.currentlyOverdue ? "Yes" : "No"}
              tone={details.currentlyOverdue ? "bad" : "good"}
            />

            <RiskBox
              label="Default History"
              value={details.hasDefaultHistory ? "Yes" : "No"}
              tone={details.hasDefaultHistory ? "bad" : "good"}
            />

            <RiskBox
              label="Multiple Active Loans"
              value={details.hasMultipleActiveLoans ? "Yes" : "No"}
              tone={details.hasMultipleActiveLoans ? "warning" : "good"}
            />
          </div>
        </ProfileCard>
          ...
          {loans.length === 0 ? (
            ...
            <div className="overflow-x-auto -mx-2">
              <table className="w-full min-w-[900px]">
                ...
                <tbody>
                  {loans.map((loan: BorrowerLoanSummary) => (
                    <tr
                      key={loan.loanId}
                      className="border-b border-slate-50 hover:bg-slate-50/70 transition-colors"
                    >
                      <TableCell strong>
                        {loan.referenceNumber ?? `#${loan.loanId}`}
                      </TableCell>
          ...
          {payments.length === 0 ? (
            ...
            <div className="overflow-x-auto -mx-2">
              <table className="w-full min-w-[1000px]">
                ...
                <tbody>
                  {payments.map((payment: BorrowerPayment) => (
                    <tr
                    ...
                    >
                      ...
                      <TableCell>
                        {formatCurrency(
                          payment.principalComponent ?? payment.principal,
                          payment.currency ?? currency,
                          locale,
                        )}
                      </TableCell>
                      ...
                      <TableCell>
                        {formatCurrency(
                          payment.interestComponent ?? payment.interest,
                          payment.currency ?? currency,
                          locale,
                        )}
                      </TableCell>
                      ...
                      <TableCell>
                        {payment.isLate || payment.onTime === false ? (
        ...
        {details.createdAt && (
          <div className="flex items-center justify-between border-t border-slate-200 pt-5 pb-6">
            <span className="text-xs font-medium text-slate-400">
              Borrower registered {formatDate(details.createdAt, locale)}
            </span>
  ...
  return (
    <td
      className={`px-3 py-4 text-sm whitespace-nowrap ${
        strong ? "font-bold text-slate-800" : "font-medium text-slate-600"
      }`}
    >
      {children}
    </td>
