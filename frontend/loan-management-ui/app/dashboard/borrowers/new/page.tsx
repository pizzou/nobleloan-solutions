...
export default function NewBorrowerPage() {
  ...
  const validate = (): boolean => {
    ...
    if (
      creditScore &&
      (Number(creditScore) < 0 || Number(creditScore) > 1000)
    ) {
    ...
    <div className="max-w-2xl space-y-6">
        ...
        <div className="p-4 rounded-xl bg-amber-50 border border-amber-200 flex items-start gap-3">
          ...
          <div className="flex-1">
            <p className="text-sm font-medium text-amber-900">
              This person already exists — matched by{" "}
              {duplicateBorrower.matchedOn}
            </p>
            <p className="text-sm text-amber-700 mt-0.5">
              {duplicateBorrower.firstName} {duplicateBorrower.lastName} is
              already a borrower. If they need another loan, add it to their
      ...
      >
        ...
        <Field
          label="National ID"
          required
          hint="(exactly 16 digits)"
          error={errors.nationalId}
        >
          <input
            value={nationalId}
            onChange={(e) =>
              setNationalId(e.target.value.replace(/[^0-9]/g, "").slice(0, 16))
            }
            inputMode="numeric"
