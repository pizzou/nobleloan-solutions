...
export default function BorrowersPage() {
  ...
  const load = useCallback(() => {
    ...
    borrowerApi
      .list(page, 20, q)
      .then((r: any) => {
        setBorrowers(r.content ?? r);
        setTotal(r.totalElements ?? (r.content ?? r).length);
      })
      .catch(console.error)
      ...
      <Card>
        {loading ? (
          ...
          <Table>
            ...
            <Tbody>
              {borrowers.length === 0 ? (
                <EmptyRow cols={9} message="No borrowers found" />
              ) : (
                borrowers.map((b: Borrower) => (
                  <Tr key={b.id}>
                    ...
                    <Td className="font-semibold text-sm">
                      {formatCurrency(b.monthlyIncome, currency, locale)}
                    </Td>
                    <Td>
                      <span
                        className={`font-bold text-sm ${(b.creditScore ?? 0) >= 700 ? "text-teal-600" : (b.creditScore ?? 0) >= 600 ? "text-yellow-600" : "text-red-500"}`}
                      >
                        {b.creditScore ?? "—"}
                      </span>
                    </Td>
