...
export default function NotificationsPage() {
  ...
  useEffect(() => {
    Promise.all([
      getLoans().catch((e) => {
        console.error("notifications: getLoans failed", e);
        return [];
      }),
      getOverduePayments().catch((e) => {
        console.error("notifications: getOverduePayments failed", e);
        return [];
      }),
      getDashboardStats().catch((e) => {
        console.error("notifications: getDashboardStats failed", e);
        return null;
      }),
      getMyNotifications().catch((e) => {
        console.error("notifications: getMyNotifications failed", e);
        return [];
      }),
    ])
        ...
        try {
          if (o.length > 0)
            n.push({
              id: "ov",
              type: "danger",
              title: `${o.length} Overdue Payment${o.length > 1 ? "s" : ""}`,
          ...
          const hr = l.filter(
            (x) => x.riskCategory === "HIGH" || x.riskCategory === "CRITICAL",
          );
          if (hr.length > 0)
            n.push({
              id: "hr",
              type: "warning",
              title: `${hr.length} High-Risk Loan${hr.length > 1 ? "s" : ""}`,
          ...
          const rate =
            s && s.totalDisbursed > 0
              ? (s.totalCollected / s.totalDisbursed) * 100
              : 0;
          if (s && rate >= 80)
            n.push({
              id: "cr-good",
              ...
              message: `Portfolio collection rate is ${rate.toFixed(0)}% — excellent performance!`,
              time: "This week",
            });
            ...
            n.push({
              ...
              message: `Collection rate is only ${rate.toFixed(0)}%. Consider sending payment reminders.`,
              time: "This week",
            });
          ...
          if (s && s.completedLoans > 0)
            n.push({
              id: "closed",
              type: "success",
              title: `${s.completedLoans} Loan${s.completedLoans > 1 ? "s" : ""} Fully Repaid`,
    ...
    <div className="space-y-5 max-w-3xl">
      ...
      <div className="space-y-3">
        ...
        {filtered.map((n) => (
          <div
            key={n.id}
            className={`rounded-2xl border p-5 ${BG[n.type]} ${n.realId && !n.read ? "ring-2 ring-offset-1 ring-teal-300" : ""}`}
          >
            <div className="flex items-start gap-4">
              <span className="text-2xl flex-shrink-0">{ICON[n.type]}</span>
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between gap-2 mb-1">
                  <p className={`font-semibold text-sm ${TXT[n.type]}`}>
                    {n.title}
                  </p>
                ...
                {n.link && (
                  <Link
                    ...
                    onClick={() => {
                      if (n.realId && !n.read)
                        markNotificationRead(n.realId).catch(() => {});
                    }}
