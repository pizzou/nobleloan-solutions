...
export default function CurrenciesPage() {
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      setRefreshMsg(
        r?.success
          ? `Updated ${r.updatedCount} currencies from ${r.source}.`
          : r?.source || "Refresh did not update any rates.",
      );
      load();
    } catch (e: any) {
    ...
    <div>
      <div className="mb-6 flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            Multi-currency support across 25 currencies · Live rates via
            exchangerate-api.com, refreshed daily
          </p>
        </div>
        <div className="text-right">
          ...
          {refreshMsg && (
            <p className="text-xs text-gray-500 mt-1.5">{refreshMsg}</p>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
        ...
        <Card className="lg:col-span-2">
          <CardHeader
            title={`Exchange Rates — Base: ${base}`}
            action={
              <Select
                value={base}
                ...
                {(Array.isArray(rates) ? rates : []).length === 0 ? (
                  <Tr>
                    <Td className="text-center py-8 text-gray-400">
                      No rates cached yet — click "Refresh Now" above to pull
                      live rates.
                    </Td>
                  </Tr>
                ) : (
                    ...
                    <Tr key={r.targetCurrency}>
                      <Td>
                        <div className="flex items-center gap-2">
                          <span className="w-10 h-6 bg-gray-100 rounded text-xs font-bold flex items-center justify-center text-gray-700">
                            {r.targetCurrency}
                          </span>
      ...
      <Card>
        <CardHeader
          ...
          subtitle="All currencies available for loan issuance"
        />
        <CardBody>
          <div className="grid grid-cols-4 sm:grid-cols-6 lg:grid-cols-9 gap-2">
            {SUPPORTED_CURRENCIES.map((c) => (
              <div
                key={c}
                className="bg-gray-50 border border-gray-200 rounded-lg p-3 text-center hover:border-teal-400 transition-colors cursor-default"
              >
