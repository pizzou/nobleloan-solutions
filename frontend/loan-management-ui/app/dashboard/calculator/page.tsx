    ...
    <div className="space-y-6 max-w-4xl">
      <div>
        ...
        <p className="text-sm text-gray-500">
          Calculate monthly payments and full amortization schedule
        </p>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-6">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
          <div>
            ...
            >
              {["USD", "EUR", "GBP", "RWF", "KES", "UGX", "TZS"].map((c) => (
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-6">
          ...
          <div className="flex rounded-full overflow-hidden h-6 mb-3">
            <div
              ...
              style={{
                width: (parseFloat(amount) / summary.total) * 100 + "%",
              }}
            >
              Principal
            </div>
          ...
          <div className="flex justify-between text-xs text-gray-500">
            <span>
              🟢 Principal:{" "}
              {((parseFloat(amount) / summary.total) * 100).toFixed(1)}%
            </span>
            <span>
              🟠 Interest:{" "}
              {((summary.interest / summary.total) * 100).toFixed(1)}%
            </span>
          </div>
        </div>
      )}
        ...
        <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
            <h2 className="font-semibold text-gray-800 text-sm">
              Amortization Schedule ({schedule.length} installments)
            </h2>
            <button
              onClick={exportCSV}
              ...
              <tbody className="divide-y divide-gray-50">
                {schedule.map((s) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-3 text-orange-500">
                      {fmt(s.interest)}
                    </td>
