...
export default function CollectionsPage() {
  ...
  const handleSync = async () => {
    setSyncing(true);
    try {
      const r = await syncCollectionsQueue();
      toast("success", typeof r === "string" ? r : "Queue synced");
      await load();
    } catch (e) {
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <div className="bg-white rounded-xl border border-gray-200 p-4">
            ...
            <p className="text-2xl font-bold text-gray-900">
              {stats.totalOpenCases}
            </p>
          </div>
          <div className="bg-white rounded-xl border border-gray-200 p-4">
            <p className="text-xs text-gray-400">Total Overdue</p>
            <p className="text-2xl font-bold text-red-600">
              {stats.totalOverdueAmount?.toLocaleString(undefined, {
                ...
                <tr key={c.id} className="hover:bg-gray-50">
                  <td className="px-4 py-3 font-medium text-gray-800">
                    ...
                    <div className="text-xs text-gray-400">
                      {c.loan?.borrower?.phone}
                    </div>
                  </td>
                  <td className="px-4 py-3 text-gray-600">
                    {c.loan?.referenceNumber}
                  </td>
                  <td className="px-4 py-3">
                    <Pill
                      label={BUCKET_LABEL[c.bucket]}
                      color={BUCKET_COLOR[c.bucket]}
                    />
                  </td>
                  <td className="px-4 py-3">
                    <Pill
                  ...
                  <td className="px-4 py-3 text-right font-semibold text-gray-800">
                    {c.loan?.currency}{" "}
                    {c.overdueAmount?.toLocaleString(undefined, {
                      maximumFractionDigits: 0,
                    })}
                  </td>
                  <td className="px-4 py-3 text-gray-600">
                  ...
                  <td className="px-4 py-3">
                    <Pill label={c.status.replace(/_/g, " ")} color="gray" />
                  </td>
        ...
        >
          ...
          >
            <h3 className="font-bold text-gray-900">
              Log action — {activeCase.loan?.borrower?.firstName}{" "}
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <button
                disabled={busy}
                onClick={() => handleAction("FIELD_VISIT")}
                className="border border-gray-300 rounded-lg py-2 text-sm font-medium hover:bg-gray-50"
              >
                🚶 Field Visit
              </button>
              <button
                ...
                onClick={() => handleAction("PROMISE_TO_PAY")}
            ...
            <button
              onClick={() => setActiveCase(null)}
              className="text-gray-500 text-sm w-full text-center pt-1"
            >
              Cancel
            </button>
          </div>
