...
export default function WebhooksPage() {
  ...
  const load = () => {
    ...
    webhookApi
      .list()
      .then((r: any) => setWebhooks(Array.isArray(r) ? r : []))
      .catch(console.error)
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setAddOpen(false);
      setForm({ url: "", description: "", subscribedEvents: ALL_EVENTS });
      load();
    } catch (err: any) {
      setMsg(err.message);
    }
  ...
  const toggleEvent = (ev: string) =>
    setForm((f) => ({
      ...f,
      subscribedEvents: f.subscribedEvents.includes(ev)
  ...
  return (
    <div>
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            Real-time event delivery to your systems via HMAC-SHA256 signed POST
            requests
          </p>
        </div>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 mb-5 text-sm text-blue-700">
        <strong>How it works:</strong> When events occur (loan approved, payment
        made, etc.), we send a POST request to your endpoint URL with a JSON
        payload. Each request is signed with{" "}
              ...
              {webhooks.length === 0 ? (
                ...
                webhooks.map((wh: WebhookEndpoint) => (
                  <Tr key={wh.id}>
                    ...
                    <Td>
                      <span
                        className={`inline-flex items-center gap-1 text-xs font-semibold px-2 py-0.5 rounded-full ${wh.active ? "bg-green-100 text-green-700" : "bg-red-100 text-red-600"}`}
                      >
                    ...
                    <Td className="text-xs text-gray-400">
                      ...
                      {wh.lastDeliveryStatus && (
                        <div
                          className={`text-[10px] mt-0.5 ${wh.lastDeliveryStatus === "SUCCESS" ? "text-teal-500" : "text-red-400"}`}
                        >
                          {wh.lastDeliveryStatus}
                        </div>
                      )}
                    ...
                    <Td
                      className={`font-semibold ${(wh.failureCount ?? 0) > 0 ? "text-red-500" : "text-gray-400"}`}
                    >
                      {wh.failureCount ?? 0}
                    </Td>
                    <Td>
      ...
      >
        <form onSubmit={handleAdd}>
            ...
            <div className="grid grid-cols-2 gap-2 mt-1">
              {ALL_EVENTS.map((ev) => (
                ...
                >
                  <input
                    type="checkbox"
                    checked={form.subscribedEvents.includes(ev)}
          ...
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 text-xs text-yellow-700 mt-2">
            🔑 A signing secret will be generated automatically. Use it to
            verify webhook authenticity in your system.
          </div>
        </form>
      </Modal>
    </div>
