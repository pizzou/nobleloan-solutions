...
const FALLBACK_TENANT: TenantConfig = {
  ...
  hero: {
    headline: "Finance With Confidence. Move Forward With Noble.",
    subtext:
      "Secure, transparent lending solutions for individuals, businesses, salaried professionals, and agriculture across Rwanda.",
  },
  services: [
    {
      title: "Personal Loan",
      description:
        "Flexible personal financing for approved household and individual needs.",
      icon: "👤",
      rate: "Flexible",
      rateType: "subject to assessment",
      maxAmount: "Subject to assessment",
      term: "Flexible terms",
    },
    {
      title: "Business Finance",
      description:
        "Working capital and expansion financing designed around your business needs.",
      icon: "🏢",
      rate: "Flexible",
      rateType: "subject to assessment",
      maxAmount: "Subject to assessment",
    ...
    },
    {
      ...
      description:
        "Financing support for approved vehicle purchases and productive assets.",
      icon: "🚗",
      rate: "Flexible",
      rateType: "subject to assessment",
      maxAmount: "Subject to assessment",
  ...
  stats: [
    { icon: "⚡", value: "24h", label: "Response target" },
    { icon: "🔒", value: "Secure", label: "Online process" },
    { icon: "📄", value: "Clear", label: "Transparent documentation" },
    { icon: "🤝", value: "Local", label: "Rwanda-focused support" },
  ],
};
...
export default function SiteLayout({
  ...
  /*
   * Render immediately with the trusted Noble fallback and then refresh
   * public tenant content in the background. This removes the full-page
   * wait caused by a cold backend while keeping the site dynamic.
   */
  ...
  useEffect(() => {
    let cancelled = false;
    const cacheKey = `noble-public-tenant:${slug}:v2`;
    const controller = new AbortController();

    /*
     * Public tenant configuration is safe to cache for the current
     * browser session. It is not authentication data.
     */
    try {
      ...
      // A cache problem must never prevent the public site from rendering.
    }
    ...
    fetch(`${API_BASE}/public/tenant/${slug}`, {
      ...
      .then((configRes) => {
        ...
        try {
          window.sessionStorage.setItem(cacheKey, JSON.stringify(nextTenant));
        } catch {
      ...
      .catch(() => {
        /*
         * Keep the already-rendered fallback/cached site visible.
         * A cold Render instance or temporary API outage must not turn
         * the public website into a blank error page.
         */
      })
      .finally(() => {
  ...
  return (
    <TenantCtx.Provider value={tenant}>
      ...
      <div className="min-h-screen bg-[#F7F9FC] font-sans text-slate-900">
        ...
        <nav className="sticky top-0 z-50 border-b border-slate-200/80 bg-white/90 shadow-[0_8px_30px_rgba(15,23,42,0.06)] backdrop-blur-xl">
