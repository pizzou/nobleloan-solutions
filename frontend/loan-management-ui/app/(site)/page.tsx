...
function IconBolt() {
  return <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M13 2 3 14h9l-1 8 10-12h-9l1-8z"/></svg>;
}
...
function IconHandshake() {
  return <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M11 17 6 12l-4 4 5 5 4-4Z"/><path d="m8 14 4-4 3 3 5-5"/><path d="M14 6h6v6"/></svg>;
}
function IconHeadset() {
  return <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 18v-6a9 9 0 0 1 18 0v6"/><path d="M21 19a2 2 0 0 1-2 2h-1a2 2 0 0 1-2-2v-3a2 2 0 0 1 2-2h3zM3 19a2 2 0 0 0 2 2h1a2 2 0 0 0 2-2v-3a2 2 0 0 0-2-2H3z"/></svg>;
}
...
export default function HomePage() {
  ...
  const pillars = [
    { Icon: IconBolt,      title: 'Fast Decisions',      desc: 'Applications reviewed by our credit team within 24 hours — no unnecessary delays.' },
    { Icon: IconShieldLg,  title: 'Secure & Regulated',  desc: 'Licensed lending, bank-grade encryption, and strict data protection standards.' },
    { Icon: IconHandshake, title: 'Transparent Terms',   desc: 'Every rate and fee is disclosed upfront — what we quote is what you pay.' },
    { Icon: IconHeadset,   title: 'Dedicated Support',   desc: 'A real loan officer assigned to your application, from submission to disbursement.' },
  ];
    ...
    <div>
      ...
      <section className="relative overflow-hidden bg-white border-b border-gray-100">
        ...
        <div className="relative max-w-7xl mx-auto px-4 py-20 md:py-28 grid md:grid-cols-2 gap-14 items-center">
          <div>
            <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full border text-xs font-bold mb-6 tracking-wide uppercase"
              style={{ borderColor: accent, color: primary, backgroundColor: accent + '14' }}>
              Licensed &amp; Regulated in {tenant.country === 'RW' ? 'Rwanda' : tenant.country}
            </div>
            <h1 className="text-4xl md:text-5xl font-bold leading-[1.15] mb-6 text-gray-900" style={serif}>
              {tenant.hero?.headline ?? "Need Cash Fast? We've Got You Covered!"}
            </h1>
            <p className="text-gray-500 text-lg leading-relaxed mb-8 max-w-lg">
              {tenant.hero?.subtext ?? 'Your trusted partner in financial support — personal, business, vehicle, salary advance, and agriculture loans, backed by a secure, fully compliant lending platform.'}
            </p>
            ...
            <div className="flex flex-wrap gap-x-8 gap-y-3">
              {['No hidden fees', 'Same-day response', 'Apply from anywhere'].map(label => (
                <div key={label} className="flex items-center gap-2 text-gray-600 text-sm font-medium">
                  <span style={{ color: accent }}><IconCheck /></span> {label}
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="rounded-2xl shadow-xl p-8 border-t-4" style={{ backgroundColor: primary, borderColor: accent }}>
            <div className="text-white/50 text-xs font-bold uppercase tracking-widest mb-1">{tenant.name}</div>
            <div className="text-white text-2xl font-bold mb-6" style={serif}>Why Clients Choose Us</div>
            <div className="space-y-5">
              {pillars.slice(0, 3).map(p => (
                <div key={p.title} className="flex items-start gap-3">
                  <div className="w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0" style={{ backgroundColor: accent, color: primary }}>
                    <p.Icon />
                  </div>
                  <div>
                    <div className="text-white font-semibold text-sm">{p.title}</div>
                    <div className="text-white/50 text-xs leading-relaxed mt-0.5">{p.desc}</div>
                  </div>
                </div>
              ))}
            </div>
            <Link href="/apply" className="block text-center mt-7 py-3 rounded-full font-bold text-sm"
      ...
      {/* ── TRUST STRIP ── */}
      <section className="py-6 border-b border-gray-100" style={{ backgroundColor: primary + '06' }}>
        <div className="max-w-7xl mx-auto px-4 flex flex-wrap items-center justify-center gap-x-10 gap-y-3 text-sm font-semibold" style={{ color: primary }}>
          <span className="flex items-center gap-2">🛡️ Regulated Institution</span>
          <span className="flex items-center gap-2">🔒 Bank-Grade Security</span>
          <span className="flex items-center gap-2">📄 Transparent Documentation</span>
      ...
      <section className="py-20 max-w-7xl mx-auto px-4">
        <div className="text-center mb-14">
          <div className="text-xs font-bold uppercase tracking-widest mb-2" style={{ color: accent }}>Our Commitment</div>
          <h2 className="text-3xl font-bold text-gray-900" style={serif}>Why Clients Choose {tenant.name}</h2>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {pillars.map(p => (
            <div key={p.title} className="text-center p-6 rounded-xl border border-gray-100 hover:shadow-md transition-shadow">
              <div className="w-14 h-14 rounded-full flex items-center justify-center mx-auto mb-4" style={{ backgroundColor: primary + '10', color: primary }}>
                <p.Icon />
              </div>
      ...
      <section className="py-20" style={{ backgroundColor: '#FAFAFA' }}>
        <div className="max-w-7xl mx-auto px-4">
          <div className="text-center mb-14">
            <div className="text-xs font-bold uppercase tracking-widest mb-2" style={{ color: accent }}>Our Products</div>
            <h2 className="text-3xl font-bold text-gray-900 mb-4" style={serif}>Lending Solutions For Every Need</h2>
            <p className="text-gray-500 text-lg max-w-2xl mx-auto">
              Tailored credit for individuals, businesses, and salaried employees across {tenant.country === 'RW' ? 'Rwanda' : tenant.country}.
            </p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {tenant.services?.map((service, i) => (
              <div key={service.title}
                ...
                <div className="absolute -top-4 left-7 w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold text-white shadow"
                  style={{ backgroundColor: accent }}>
                  {String(i + 1).padStart(2, '0')}
                </div>
                <h3 className="text-lg font-bold text-gray-900 mb-2" style={serif}>{service.title}</h3>
                <p className="text-gray-500 text-sm leading-relaxed mb-5">{service.description}</p>
                <div className="flex items-center justify-between text-xs mb-5 pb-5 border-b border-gray-100">
                  <span className="font-bold px-3 py-1.5 rounded-full" style={{ backgroundColor: primary + '12', color: primary }}>
      ...
      <section className="py-20 max-w-5xl mx-auto px-4">
        <div className="rounded-2xl border border-gray-200 shadow-sm overflow-hidden grid md:grid-cols-2">
          <div className="p-10 text-white flex flex-col justify-center" style={{ backgroundColor: primary }}>
            <div className="text-xs font-bold uppercase tracking-widest mb-3" style={{ color: accent }}>Plan Ahead</div>
            <h2 className="text-2xl font-bold mb-4" style={serif}>Estimate Your Repayment</h2>
            <p className="text-white/55 text-sm leading-relaxed">
              Use our calculator to get an instant estimate of your monthly repayment. Final rates and terms are confirmed after credit assessment.
            </p>
          </div>
      ...
      <section className="py-20" style={{ backgroundColor: '#FAFAFA' }}>
        <div className="max-w-3xl mx-auto px-4">
          <div className="text-center mb-14">
            <div className="text-xs font-bold uppercase tracking-widest mb-2" style={{ color: accent }}>Our Process</div>
            <h2 className="text-3xl font-bold text-gray-900" style={serif}>How to Get a Loan</h2>
          </div>
          <div className="relative">
            <div className="absolute left-5 top-2 bottom-2 w-[2px]" style={{ backgroundColor: primary + '20' }} />
            {[
              ...
              <div key={item.step} className="relative flex items-start gap-6 pb-10 last:pb-0">
                <div className="w-10 h-10 rounded-full flex items-center justify-center text-white font-bold text-sm flex-shrink-0 relative z-10"
      ...
      {tenant.team && tenant.team.length > 0 && (
        <section className="py-20 max-w-7xl mx-auto px-4">
          <div className="text-center mb-14">
            <div className="text-xs font-bold uppercase tracking-widest mb-2" style={{ color: accent }}>Our People</div>
            <h2 className="text-3xl font-bold text-gray-900" style={serif}>Meet the {tenant.name} Team</h2>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {tenant.team.map(member => (
              <div key={member.name} className="text-center p-6 rounded-xl border border-gray-100 hover:shadow-md transition-shadow">
      ...
      {tenant.testimonials && tenant.testimonials.length > 0 && (
        <section className="py-20 max-w-7xl mx-auto px-4">
          <div className="text-center mb-14">
            <div className="text-xs font-bold uppercase tracking-widest mb-2" style={{ color: accent }}>Client Stories</div>
            <h2 className="text-3xl font-bold text-gray-900" style={serif}>What Our Clients Say</h2>
          </div>
      ...
      <section className="py-16 mx-4 md:mx-auto max-w-7xl mb-16">
        <div className="rounded-2xl p-12 text-center relative overflow-hidden border-2"
          style={{ backgroundColor: accent + '10', borderColor: accent + '40' }}>
          <h2 className="text-3xl md:text-4xl font-bold mb-4 relative z-10 text-gray-900" style={serif}>
            Ready to Take the Next Step?
          </h2>
          <p className="text-gray-600 text-lg mb-8 relative z-10 max-w-xl mx-auto">
            Apply today and get a response within 24 hours. No hidden fees, no surprises — just honest lending.
          </p>
          <Link href="/apply"
...
function StatCard({ stat, primary, delay }: { stat: { icon?: string; value: string; label: string }; primary: string; delay: number }) {
  ...
  return (
    <div ref={ref} className={`reveal reveal-delay-${Math.min(delay + 1, 4)} ${visible ? 'reveal-visible' : ''} text-center px-4`}>
      ...
      <div className="text-xs md:text-sm text-gray-500 font-semibold mt-1">{stat.label}</div>
    </div>
  );
}
...
function TestimonialCard({ t, primary, accent, delay }: { t: { name: string; role: string; text: string; rating?: number }; primary: string; accent: string; delay: number }) {
  ...
  return (
    <div ref={ref} className={`reveal reveal-delay-${Math.min(delay + 1, 4)} ${visible ? 'reveal-visible' : ''}
      card-lift bg-white rounded-xl p-6 border border-gray-100 border-t-4`} style={{ borderTopColor: accent }}>
