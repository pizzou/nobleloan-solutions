...
export default function SigningPage() {
  ...
  const load = useCallback(() => {
    ...
    setError("");
    axios
      .get(`${API_BASE}/public/esignature/${token}`)
      .then((r) => setView(r.data?.data ?? r.data))
      .catch((e) =>
        setError(
          e.response?.data?.error ||
            e.response?.data?.message ||
            "This signing link could not be found or has expired.",
        ),
      )
  ...
  const handleSign = async () => {
    ...
    setError("");
    try {
      await axios.post(`${API_BASE}/public/esignature/${token}/sign`, {
        otp,
        fullName,
      });
      setSuccess(true);
      ...
      setError(
        e.response?.data?.error ||
          e.response?.data?.message ||
          "Could not verify — please check the code and try again.",
      );
    } finally {
      setBusy(false);
    }
  ...
  const handleDecline = async () => {
    if (!confirm("Are you sure you want to decline signing this agreement?"))
      return;
    setDeclining(true);
    setError("");
    try {
      await axios.post(`${API_BASE}/public/esignature/${token}/decline`, {
        reason: "Declined by borrower",
      });
      load();
    } catch (e: any) {
      setError(
    ...
    <div className="min-h-screen bg-gray-50 py-10 px-4">
      <div className="max-w-2xl mx-auto">
        <div className="text-center mb-6">
          <div className="inline-flex w-12 h-12 bg-[#0D6B3E] rounded-xl items-center justify-center text-white font-black text-lg mb-3 shadow-md">
            ✍️
          </div>
        ...
        {alreadyDone ? (
          <div className="bg-white rounded-2xl border border-gray-200 p-8 text-center shadow-sm animate-fadeIn">
            ...
            <p className="text-sm text-gray-500">
              Thank you{view?.borrowerName ? `, ${view.borrowerName}` : ""}.
              Your signed agreement has been recorded and your loan officer has
              been notified.
            </p>
          </div>
        ) : declined ? (
          <div className="bg-white rounded-2xl border border-gray-200 p-8 text-center shadow-sm">
            ...
            <p className="text-sm text-gray-500">
              You've declined to sign this agreement. Your loan officer will be
              in touch.
            </p>
          </div>
        ) : expired ? (
          <div className="bg-white rounded-2xl border border-gray-200 p-8 text-center shadow-sm">
            ...
            <p className="text-sm text-gray-500">
              This signing link has expired. Please ask your loan officer to
              send a new one.
            </p>
          </div>
        ) : (
            ...
            <div className="bg-white rounded-2xl border border-gray-200 p-6 space-y-4 shadow-sm">
              ...
              {resendSuccess && (
                <div className="bg-green-50 border border-green-200 text-green-800 text-xs rounded-lg px-3 py-2 font-semibold animate-fadeIn">
                  📨 A fresh 6-digit verification code has been successfully
                  dispatched to your email!
                </div>
              )}
              ...
              <div>
                <label className="text-xs font-bold text-gray-600">
                  Verification code (sent by Email/SMS)
                </label>
                <input
                  value={otp}
        ...
        <p className="text-center text-x-[10px] text-gray-400 mt-6 font-medium">
          Protected electronic signature · IP address and timestamp recorded for
          verification
        </p>
      </div>
    </div>
