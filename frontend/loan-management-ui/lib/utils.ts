...
export const LOAN_TYPE_META: Record<
  string,
  { icon: string; label: string; rate: number }
> = {
  ...
  TRADE_FINANCE: {
    icon: "📦",
    label: "Contract / Invoice Financing",
    rate: 13,
  },
...
export function calcLoan(
  ...
  if (!Number.isFinite(monthlyRate) || monthlyRate < 0)
    throw new Error("Monthly rate must be non-negative");
