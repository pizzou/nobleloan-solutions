...
export default function CameraCapture({
  ...
  useEffect(() => {
    ...
    navigator.mediaDevices
      ...
      .catch(() =>
        setError(
          "We couldn't access your camera. Please check your browser permissions, or use the file upload option instead.",
        ),
      );
  ...
  const capture = () => {
    ...
    canvas.toBlob(
      (blob) => {
        if (!blob || blob.size <= 0) {
          setError(
            "The camera could not create a valid photo. Please retake the selfie.",
          );
          return;
        }
        setError("");
        ...
        <div className="p-5">
          {error ? (
            <div>
              ...
              <label
                className="block text-center py-3 rounded-md text-sm font-bold text-white cursor-pointer"
            ...
            <>
              <p className="text-xs text-gray-500 mb-3">
                Face the camera in good lighting, remove sunglasses/hats, and
