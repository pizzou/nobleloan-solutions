...
export default function Sidebar({
  ...
  /*
   * Keep the sidebar lightweight:
   * - fetch both counters concurrently;
   * - refresh once per minute instead of every 30 seconds;
   * - pause background polling when the tab is hidden.
   */
  useEffect(() => {
    if (!user) return;
    ...
    const loadUnreadCounts = async () => {
      ...
      const [notificationResult, messageResult] = await Promise.allSettled([
        getUnreadCount(),
        contactMessageApi.unreadCount(),
      ]);

      if (!active) return;

      if (notificationResult.status === "fulfilled") {
        setUnread(Number(notificationResult.value?.count || 0));
      }

      if (messageResult.status === "fulfilled") {
        setUnreadMessages(Number(messageResult.value?.count || 0));
      }
    };
  ...
  return (
    <aside
      className={`
        ...
        border-white/10
        bg-[#07152A]
        shadow-[12px_0_40px_rgba(7,21,42,0.12)]
