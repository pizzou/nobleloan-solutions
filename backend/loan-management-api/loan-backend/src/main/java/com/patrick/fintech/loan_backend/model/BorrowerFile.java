...
public class BorrowerFile {
    ...
    /**
     * True when this file was uploaded by the applicant from the public apply/track
     * flow, not by staff.
     */
    ...
    /**
     * Loan officer's note on this specific document, e.g. why it was rejected or
    ...
    /**
     * Denormalized name of whoever last verified/rejected this file — avoids a
     * lazy-loading
    ...
    // No @Lob here on purpose — with Postgres, @Lob on a byte[] field makes
    // Hibernate
    ...
    // state and was throwing "cannot execute lo_creat() in a read-only
    // transaction".
    // Hibernate 6 maps a plain byte[] to bytea correctly without @Lob.
    // @JsonIgnore — this is raw file content; list/detail endpoints must never
    // serialize
    // it into JSON (huge payloads, not even renderable as an image). Actual file
    // bytes
    // only ever leave the server through BorrowerFileController's dedicated
    // download/preview endpoints, which read it straight from the entity, not via
    // Jackson.
    @JsonIgnore
