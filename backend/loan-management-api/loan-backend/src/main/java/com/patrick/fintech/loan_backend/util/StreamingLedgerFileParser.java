...
public final class StreamingLedgerFileParser {
            ...
            while ((line = LedgerFileParser.readLogicalLine(reader)) != null) {
                ...
                for (int i = 0; i < headers.size(); i++) {
                    row.put(
                            headers.get(i),
                            i < cells.size()
                                    ? normalizeStreamValue(cells.get(i))
                                    : "");
                }
                ...
                try {
                    ...
                    throw new IOException(
                            "Import row application failed at row "
                                    + physicalRow
                                    + ".",
                            e);
                }
     ...
     * IMPORTANT:
     *
     * Apache POI's SheetIterator must be advanced with next() BEFORE calling
     * getSheetName().
     *
     * Calling getSheetName() before next() causes:
     *
     * java.lang.NullPointerException:
     * Cannot invoke XSSFSheetRef.getName()
     * because xssfSheetRef is null
     */
    private static long streamXlsx(
        ...
        try (OPCPackage packageHandle = OPCPackage.open(input)) {
            ...
            ReadOnlySharedStringsTable sharedStrings =
                    new ReadOnlySharedStringsTable(packageHandle);

            String authoritativeSheet =
                    findAuthoritativeMonthlySheet(
                            reader,
                            styles,
                            sharedStrings);
            ...
            if (authoritativeSheet != null
                    && !authoritativeSheet.isBlank()) {
                ...
                while (sheets.hasNext()) {
                     ...
                     * getSheetName() is valid only after next()
                     * has initialized the current sheet reference.
                     */
            ...
            /*
             * No monthly worksheet was detected.
             * Preserve the generic fallback behaviour for standard/credit
             * workbooks.
             */
     ...
     * IMPORTANT:
     *
     * SheetIterator.next() MUST happen before getSheetName().
     */
    private static String findAuthoritativeMonthlySheet(
        ...
        while (sheets.hasNext()) {
            ...
            try (InputStream sheetInput = sheets.next()) {
                ...
                /*
                 * Only now is getSheetName() safe.
                 */
                String sheetName = sheets.getSheetName();

                MonthlySheetProbeHandler probe =
                        new MonthlySheetProbeHandler(
                                LAYOUT_SCAN_ROWS);
                ...
                int monthlyRows =
                        probe.getMonthlyRows();

                if (monthlyRows <= 0) {
                    continue;
                }

                boolean hasPortfolioName =
                        clean(sheetName)
    ...
    /**
     * Lightweight first-pass classifier.
     *
     * It intentionally does not map or retain rows; it only counts rows that
     * structurally match the Noble Loan monthly portfolio layout.
     */
    ...
    private static final class StreamingHandler
        ...
        private final List<Map<Integer, String>> probeRows =
                new ArrayList<>();

        private final List<Long> probeRowNumbers =
                new ArrayList<>();
        ...
        private StreamingHandler(
            ...
            this.pendingCreditRows =
                    pendingCreditRows;

            this.authoritativeLedgerSeen =
                    authoritativeLedgerSeen;
        }
        ...
        public void endRow(int rowNum) {
            ...
            if (!probeComplete) {
                ...
                if (layout != Layout.UNKNOWN
                    ...
                    /*
                     * Re-process probe rows through the selected layout.
                     * This is bounded to 60 rows and therefore remains safe
                     * for large workbooks.
                     */
                    for (int i = 0;
                         i < probeRows.size();
                        ...
                        Map<Integer, String> probe =
                                probeRows.get(i);

                        long rowNumber =
                                probeRowNumbers.get(i);
        ...
        private void emitMapped(
            ...
            try {
                ...
                throw new ImportRowRuntimeException(
                        rowNumber,
                        e);
            }
        }

        private Map<String, String> mapStandard(
                Map<Integer, String> row) {
        ...
        private Map<String, String> mapMonthly(
                Map<Integer, String> row) {
             ...
             * paid amounts. They are not Noble Loan's one-time application
             * fee fields. The actual historical application fee is column 5.
             */
            BigDecimal applicationOutstanding =
                    BigDecimal.ZERO;

            BigDecimal applicationPaid =
            ...
            out.put(
                    "start_date",
                    normalizeDate(row.get(14)));

            out.put(
                    "next_due_date",
                    normalizeDate(row.get(15)));
            ...
            out.put(
                    "total_repayable",
                    totalRepayable.toPlainString());

            out.put(
                    "principal_paid",
                    principalPaid.toPlainString());

            out.put(
                    "interest_paid",
                    interestPaid.toPlainString());

            out.put(
                    "interest_outstanding",
                    interestOutstanding.toPlainString());

            out.put(
                    "management_fee_paid",
                    managementPaid.toPlainString());

            out.put(
                    "total_management_fee_balance",
                    managementOutstanding.toPlainString());
            ...
            out.put(
                    "notes",
                    "Imported from Noble Loan historical portfolio workbook");

            return out;
        }

        private boolean looksLikeCredit(
                Map<Integer, String> row) {
        ...
        private Map<String, String> mapCredit(
                Map<Integer, String> row) {
    ...
    private static final class ImportRowRuntimeException
            extends RuntimeException {

        private final long rowNumber;

        private ImportRowRuntimeException(
                long rowNumber,
                Exception cause) {
    ...
    private static String normalizeStreamValue(
            String value) {
    ...
    private static String canonicalHeader(
        ...
        return switch (normalized) {
            ...
            case "application_fee_outstanding",
                 "applicationfee_outstanding" ->
                    "application_fee_outstanding";
     ...
     * portfolio layout.
     */
    private static boolean looksLikeMonthlyRow(
            Map<Integer, String> row) {
    ...
    private static int headerScore(
            List<String> headers) {
    ...
    private static Map<String, String> canonicalizeRow(
        ...
        if ((row.get("first_name") == null
                || row.get("first_name").isBlank())
                && row.get("names") != null
                && !row.get("names").isBlank()) {

            String[] names =
                    splitName(row.get("names"));
    ...
    private static boolean positiveDecimal(
            String value) {
    ...
    private static boolean integerInRange(
            String value,
            int min,
            int max) {
    ...
    private static String normalizeRate(
            String value) {
    ...
    private static String normalizeLoanType(
            String value) {
