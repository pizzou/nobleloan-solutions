...
public class LoanRestructuringController {
    ...
    public ResponseEntity<ApiResponse<Loan>> restructure(@PathVariable Long loanId,
            @RequestBody Map<String, Object> body) {
        var u = currentUserUtil.getCurrentUser();
        return ResponseEntity.ok(ApiResponse.ok("Loan restructured",
                svc.restructure(loanId, u.getOrganization().getId(), u,
                        Integer.parseInt(body.get("newDurationMonths").toString()),
                        body.containsKey("newInterestRate") ? Double.parseDouble(body.get("newInterestRate").toString())
                                : null,
    ...
    @PreAuthorize("hasAnyRole('ADMIN','MANAGER')")
    public ResponseEntity<ApiResponse<Loan>> writeOff(@PathVariable Long loanId,
            @RequestBody Map<String, String> body) {
        var u = currentUserUtil.getCurrentUser();
        return ResponseEntity.ok(ApiResponse.ok("Loan written off",
                svc.writeOff(loanId, u.getOrganization().getId(), u, body.getOrDefault("reason", "Uncollectible"))));
    }

    @PostMapping("/moratorium")
    @PreAuthorize("hasAnyRole('ADMIN','MANAGER')")
    public ResponseEntity<ApiResponse<Loan>> moratorium(@PathVariable Long loanId,
            @RequestBody Map<String, Object> body) {
        var u = currentUserUtil.getCurrentUser();
        return ResponseEntity.ok(ApiResponse.ok("Moratorium granted",
                svc.grantMoratorium(loanId, u.getOrganization().getId(), u,
