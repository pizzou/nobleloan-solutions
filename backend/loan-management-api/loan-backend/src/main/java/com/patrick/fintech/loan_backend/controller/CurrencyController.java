...
public class CurrencyController {
    ...
    @GetMapping
    public ResponseEntity<ApiResponse<Object>> rates(
            @RequestParam(defaultValue = "USD") String base) {
        return ResponseEntity.ok(ApiResponse.safe(currencyService.getRatesForBase(base)));
    }
    ...
    public ResponseEntity<ApiResponse<List<String>>> supported() {
        return ResponseEntity.ok(ApiResponse.safe(CurrencyService.SUPPORTED_CURRENCIES));
    }
    ...
    public ResponseEntity<ApiResponse<CurrencyService.RefreshResult>> refresh() {
        ...
        return ResponseEntity.ok(result.success()
                ? ApiResponse.safe("Rates refreshed from " + result.source(), result)
                : ApiResponse.safe("Refresh did not update rates: " + result.source(), result));
    }
    ...
    public ResponseEntity<ApiResponse<Map<String, Object>>> status() {
        ...
        return ResponseEntity.ok(ApiResponse.safe(status));
    }
}
