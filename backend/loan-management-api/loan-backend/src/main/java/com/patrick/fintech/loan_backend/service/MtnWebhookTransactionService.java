...
public class MtnWebhookTransactionService {
        ...
        /*
         * THIS IS THE IMPORTANT TRANSACTION BOUNDARY.
         *
         * This method belongs to a separate Spring bean.
         *
         * Therefore calls from MtnMobileMoneyService go through
         ...
         * findByIdForUpdate()
         *
         * executes while a real database transaction is active.
         */
        @Transactional
        ...
        public PaymentGatewayResponse confirmPayment(
                 ...
                 */
                var loan = loanRepo.findByIdForUpdate(
                                loanId)
                                .orElseThrow(
                ...
                if (!sandbox && !"MTN_VERIFIED_WEBHOOK".equals(confirmationSource)) {

                        return PaymentGatewayResponse.failed(
                                        "MTN production callback requires provider-side transaction verification",
                                        MTN_PROVIDER);
                }
                ...
                BigDecimal outstandingBalance = payment.getOutstandingAfterDecimal();

                if (outstandingBalance == null) {

                        outstandingBalance = loan.getOutstandingBalanceDecimal();
                }
                ...
                outstandingBalance = outstandingBalance.setScale(
                                2,
                                RoundingMode.HALF_UP);
