...
public class UserController {
    ...
    public ResponseEntity<ApiResponse<Map<String, Object>>> create(@RequestBody RegisterRequest req) {
        // Never trust a tenant id supplied by the browser. User creation is always
        // scoped
        ...
        auditService.log(user.getOrganization(), currentUserUtil.getCurrentUser(), "USER_CREATED", "USER",
                ...
                "Created user " + user.getName() + " (" + user.getEmail() + ") — credentials emailed",
                null, null, "User Management");
        return ResponseEntity
                .ok(ApiResponse.ok("User created — login details have been emailed to them", safeUser(user)));
    }
    ...
    public ResponseEntity<ApiResponse<Map<String, Object>>> update(
        ...
        // Previously this endpoint had no authorization check at all — any
        // authenticated user
        // could edit any other user's name/password by guessing an id, even across
        // organizations.
    ...
    public ResponseEntity<ApiResponse<Map<String, Object>>> changeRole(
        ...
        // Was: user.setRole(role); userService.update(id, user) — update() only ever
        // copies
        // the `name` field onto a fresh re-fetch of the user, so the role set here was
        // silently discarded every time. This endpoint returned "success" but never
        // actually
    ...
    public ResponseEntity<ApiResponse<Void>> delete(@PathVariable Long id) {
        ...
        if (!target.getOrganization().getId().equals(orgId))
            throw new RuntimeException("Access denied");
        // Was a hard userService.delete(id) -> userRepository.deleteById(id). Every
        // user this
        // platform's ever used has rows elsewhere (loans, audit logs, uploads,
        // approvals...)
        // pointing at them by foreign key with no cascade rule, so that delete always
        // failed
        // with a database constraint violation — surfaced to the admin as a generic
        // "conflicts
        // with an existing record" error. Deactivating instead blocks their login and
        // removes
        // them from active use without breaking referential integrity or the audit
        // trail.
        ...
        auditService.log(target.getOrganization(), currentUserUtil.getCurrentUser(), "USER_DEACTIVATED", "USER",
                ...
                null, null, "User Management");
        return ResponseEntity
                .ok(ApiResponse.ok("User deactivated — their login is disabled and their history is preserved"));
    }

    /**
     * Undoes delete()/deactivate() above — an admin's only way to walk back a
     * deactivation
    ...
    private Map<String, Object> safeUser(User u) {
        ...
        m.put("role", u.getRole() != null
                ? Map.of("id", u.getRole().getId(), "name", u.getRole().getName())
                : null);
        m.put("organization", u.getOrganization() != null
                ? Map.of("id", u.getOrganization().getId(), "name", u.getOrganization().getName())
                : null);
