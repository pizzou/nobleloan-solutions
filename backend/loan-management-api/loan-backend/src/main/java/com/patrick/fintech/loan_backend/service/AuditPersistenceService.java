...
public class AuditPersistenceService {
    ...
    @Value("${app.audit.geolocation-enabled:false}")
    private boolean geolocationEnabled;

    /**
     * Short-timeout client dedicated to geolocation lookups so a slow/unreachable
     * geo provider can never back up the shared application RestTemplate or the
     * async pool.
     */
    ...
    // Both overloads are @Async entry points in their own right — per the class
    // javadoc,
    // one calling the other via `this` would bypass the proxy and run inline, so
    // instead
    ...
    private void doPersist(Long orgId, Long actorId, String action, String entityType, String entityId,
        ...
        try {
            ...
            // the append-only chain. PostgreSQL transaction advisory locks release
            // automatically when this transaction commits or rolls back.
            entityManager.createNativeQuery(
                    "select pg_advisory_xact_lock(hashtext(:lockKey))")
                    .setParameter("lockKey", "loansaas:audit-chain")
                    .getSingleResult();
    ...
    // ---- entityType -> human-readable module/page, kept in sync with V19
    ...
    private String parseOs(String ua) {
        ...
        if (ua.contains("Windows")) {
            ...
            if (m.find())
                return WINDOWS_NAMES.getOrDefault(m.group(1), "Windows");
            return "Windows";
        }
