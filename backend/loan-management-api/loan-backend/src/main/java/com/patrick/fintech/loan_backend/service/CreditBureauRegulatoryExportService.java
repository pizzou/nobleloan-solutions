 ...
 * Bounced Cheques.
 *
 * All organization-owned child records are retrieved using organization-scoped
 * repository methods to preserve tenant isolation.
 */
@Service
...
public class CreditBureauRegulatoryExportService {
    ...
    private static final DateTimeFormatter CRB_DATE =
    ...
    private static final String[] SHAREHOLDER_HEADERS = {
            ...
            "Surname/ Institution Name",
            "Forename or Initial 1/ Trading Name",
            "Forename or Initial 2",
            "Forename or Initial 3",
            "National ID Number/ Company Reg No",
            "Passport No",
            "Nationality",
    ...
    private static final String[] GUARANTOR_HEADERS = {
            ...
            "Nationality",
            "Date of Birth",
            "Place Of Birth/ Company Registration Date",
            "Postal Address Line 1 Number",
            "Postal Address Line 2 Postal Code",
            "Town",
            "Country",
    ...
    /**
     * Loads guarantors using both loan ID and organization ID.
     *
     * This method intentionally does not use:
     *
     * guarantorRepository.findByLoan_Id(loanId)
     *
     * because that query is not tenant-scoped.
     */
    private List<Guarantor> safeGuarantors(
            Long loanId,
            Long organizationId) {
    ...
    /**
     * Loads collateral using both loan ID and organization ID.
     *
     * This method intentionally does not use:
     *
     * collateralRepository.findByLoan_Id(loanId)
     *
     * because that query is not tenant-scoped.
     */
    private List<Collateral> safeCollaterals(
            Long loanId,
            Long organizationId) {
    ...
    private void writeConsumerRow(
        ...
        String[] values =
                new String[CONSUMER_HEADERS.length];

        values[0] =
                salutationFromGender(borrower.getGender());
        ...
        values[8] =
                borrower.getTaxIdentificationNumber();
        ...
        values[41] =
                borrower.getMonthlyIncomeDecimal() == null
                        ? null
                        : "MONTHLY";
        ...
        values[52] =
                date(disbursementDate(loan));

        values[53] =
                date(
                        loan.getUpdatedAt() != null
                                ? loan.getUpdatedAt().toLocalDate()
                                : null
                );
        ...
        values[65] =
                closed(loan)
                        ? date(closedDate(loan))
                        : null;

        values[66] =
                date(loan.getLastPaymentDate());

        values[67] =
                annualizedInterestRate(loan);

        values[68] =
                date(firstPaymentDate(payments));
        ...
        values[72] =
                date(loan.getApprovedAt());

        values[73] =
                date(loan.getMaturityDate());
        ...
        /*
         * CRB numeric columns are deliberately stored as
         * actual numeric Excel cells rather than strings.
         */
        setNumeric(
                row,
                40,
        ...
        setNumeric(
                row,
                67,
                annualizedRateDecimal(loan),
                styles.number
        );
    }

    private void writeGuarantorRow(
            Row row,
            Loan loan,
            Guarantor guarantor,
            Styles styles) {
        ...
        String[] values =
                new String[GUARANTOR_HEADERS.length];
    ...
    private void writeCollateralRow(
            Row row,
            Loan loan,
        ...
        String[] values =
                new String[COLLATERAL_HEADERS.length];

        values[0] =
                loan.getReferenceNumber();

        values[1] =
                collateral.getType() == null
                        ? null
                        : collateral.getType().name();

        values[2] =
                moneyText(
                        collateral.getEstimatedValueDecimal()
                );

        values[3] =
                date(
                        collateral.getUpdatedAt() != null
                                ? collateral.getUpdatedAt().toLocalDate()
                                : null
                );

        values[4] =
                date(
                        collateral.getInsuranceExpiryDate()
                );
    ...
    private void writeLoanCollateralFallback(
            Row row,
            Loan loan,
            Styles styles) {
        ...
        values[2] =
                moneyText(
                        loan.getCollateralValueDecimal()
                );
    ...
    private boolean isMandatoryHeader(
            String sheet,
            int column) {
        ...
        if ("Consumer".equals(sheet)) {
            ...
            int[] optional = {
                    3, 4, 8, 9, 10, 11, 13,
                    17, 18, 20, 21, 22, 28, 29,
                    33, 34, 35, 36, 37, 38,
                    40, 41, 42, 43, 45, 50, 58
            };
    ...
    private double[] referenceWidths(
            String sheetName,
            int columnCount) {
        ...
        if ("Consumer".equals(sheetName)) {
            ...
            widths = new double[]{
                    9.08, 8.26, 18.35, 13, 13, 18.99,
                    10.53, 9.62, 6.53, 16.08, 20.17,
                    22.17, 12.17, 15.44, 6.81, 11.26,
                    11.9, 25.62, 28.72, 19.99, 13,
                    24.81, 25.44, 22.17, 20.9, 20.35,
                    18.17, 7.53, 12.35, 13.72, 14.26,
                    14.81, 15.17, 8.35, 14.08, 21.35,
                    13, 13.62, 15.81, 9.81, 6.99,
                    16.08, 11.17, 12.99, 14.35, 17.72,
                    11.72, 13.72, 18.17, 13.17, 19.35,
                    12.9, 19.17, 12.08, 13.72, 15.26,
                    26.26, 14.08, 13.9, 33.08, 31.35,
                    20.72, 15.44, 19.81, 13.62, 10.81,
                    16.53, 11.72, 16.72, 6.35, 8.35,
                    14.9, 12.53, 17.08
            };
            ...
            widths = new double[]{
                    14.62, 12.35, 6.53, 6.72, 15.17,
                    23.81, 18.81, 20.81, 25.62, 28.72,
                    19.99, 13, 24.81, 25.44, 22.17,
                    20.9, 20.35, 18.17, 7.35, 12.35,
                    18.08, 10.26, 13, 13, 13, 13,
                    9.35, 13, 14.35, 17.72, 11.72,
                    12.99, 11.44, 13.17, 19.35, 12.9,
                    11.53, 12.08, 13.72, 15.26, 26.26,
                    14.08, 13.9, 22.17, 31.35, 20.72,
                    15.08, 19.81, 13.62, 10.72, 16.53,
                    11.72, 16.72, 6.35, 14.9, 12.53,
                    17.08
            };
            ...
            widths = new double[]{
                    14.35, 15.44, 15.72, 18.44, 23.53,
                    31.44, 18.35, 13, 33.17, 10.53,
                    9.62, 35.81, 11.9, 25.62, 28.72,
                    5.17, 7.35
            };
            ...
            widths = new double[]{
                    15.72, 10.17, 13, 19.99, 13, 13,
                    18.81, 11.99, 10.53, 12.26, 13.17,
                    28.53, 32.53, 5.81, 8.17, 15.17
            };

        } else if ("Guarantors".equals(sheetName)) {

            widths = new double[]{
                    14.35, 13.81, 23.53, 31.44, 18.35,
                    13, 33.17, 10.53, 9.62, 11.26,
                    36.44, 25.62, 28.72, 5.17, 7.35,
                    14.26, 14.81, 15.17
            };
            ...
            widths = new double[]{
                    15.72, 14.53, 15.26, 28.44, 20.72
            };

        } else {

            widths = new double[]{
                    14.35, 8.81, 22.99, 30.9, 18.35,
                    13, 32.62, 16.99, 9.62, 35.81,
                    11.9, 25.62, 28.72, 5.17, 7.35,
                    14.17, 11.44, 12.9, 8.26, 7.81,
                    21.9, 23.44
            };
        }
    ...
    private LocalDate closedDate(Loan loan) {
        ...
        return loan.getLastPaymentDate() != null
    ...
    private String arrearsClassification(
            Integer days,
            LoanStatus status) {
    ...
    private BigDecimal annualizedRateDecimal(
            Loan loan) {
        ...
        /*
         * Noble Loan stores the contractual lending rate monthly.
         *
         * CRB receives the corresponding annual nominal rate.
         *
         * The stored monthly rate itself is not modified.
         */
    ...
    private BigDecimal latestPaidAmount(
            List<Payment> payments) {
    ...
    private String salutationFromGender(
            String gender) {
    ...
    private String normalizeMaritalStatus(
            String status) {
    ...
    private static final class Styles {
        ...
        private Styles(Workbook workbook) {
            ...
            mandatoryFont.setFontName(
                    "Verdana"
            );

            mandatoryFont.setFontHeightInPoints(
                    (short) 9
            );

            mandatoryFont.setColor(
            ...
            optionalFont.setFontName(
                    "Verdana"
            );

            optionalFont.setFontHeightInPoints(
                    (short) 9
            );

            optionalFont.setColor(
            ...
            bodyFont.setFontHeightInPoints(
                    (short) 11
            );
