...
public class FinancialMonitoringService {
    ...
    public FinancialMonitoringService(
        ...
        Gauge.builder("loansaas_financial_reconciliation_failed_organizations", state, s -> s.getFailedOrganizations())
                ...
                .register(meterRegistry);
        Gauge.builder("loansaas_financial_reconciliation_max_difference", state, s -> s.getLastMaximumDifference().doubleValue())
                .description("Largest financial reconciliation difference from the latest run")
                .register(meterRegistry);
    }

    @Scheduled(
            fixedDelayString = "${app.monitoring.reconciliation-interval-ms:300000}",
            initialDelayString = "${app.monitoring.reconciliation-initial-delay-ms:60000}")
        ...
        for (Organization organization : organizations) {
            ...
            try {
                FinancialReconciliationService.ReconciliationReport report =
                        reconciliationService.reconcile(organization.getId());
