...
public class OrganizationController {
    ...
    public ResponseEntity<ApiResponse<Map<String, Object>>> getMyOrg() {
        ...
        m.put("accentColor", org.getAccentColor());
        ...
        m.put("tagline", org.getTagline());
        m.put("mission", org.getMission());
        m.put("vision", org.getVision());
        m.put("foundedYear", org.getFoundedYear());
        m.put("mapUrl", org.getMapUrl());
        m.put("facebookUrl", org.getFacebookUrl());
        m.put("instagramUrl", org.getInstagramUrl());
        m.put("linkedinUrl", org.getLinkedinUrl());
        m.put("twitterUrl", org.getTwitterUrl());
    ...
    public ResponseEntity<ApiResponse<Organization>> updateMyOrg(@RequestBody Map<String, Object> body) {
        ...
        if (body.containsKey("foundedYear") && body.get("foundedYear") != null) {
            try {
                org.setFoundedYear(Integer.valueOf(body.get("foundedYear").toString()));
        ...
        if (body.containsKey("hero") && body.get("hero") instanceof Map<?, ?> hero) {
            if (hero.get("headline") != null)
                org.setHeroHeadline(hero.get("headline").toString());
            if (hero.get("subtext") != null)
                org.setHeroSubtext(hero.get("subtext").toString());
        }
        // Repeatable content lists — stored as JSON, editable in full from the
        // dashboard
    ...
    private void setJsonIfPresent(Map<String, Object> body, String key, java.util.function.Consumer<String> setter) {
        if (!body.containsKey(key) || body.get(key) == null)
            return;
        try {
            setter.accept(objectMapper.writeValueAsString(body.get(key)));
        } catch (Exception e) {
            throw new RuntimeException("Invalid " + key + " content: " + e.getMessage());
        }
    }
