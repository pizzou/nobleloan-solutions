...
public final class PiiCrypto {
        ...
        if (configured != null && !configured.isBlank()) {
            ...
            if ("production".equalsIgnoreCase(environment)
                ...
                throw new IllegalStateException(
                        "APP_ENCRYPTION_KEY is required in production; refusing to start PII encryption with a fixed development key.");
            }
    ...
    private static SecretKeySpec keyFromBase64(String base64, String varName) {
        ...
        if (keyBytes.length != 32) {
            throw new IllegalStateException(
                    varName + " must decode to exactly 32 bytes (AES-256) — got " + keyBytes.length);
        }
    ...
    private static SecretKeySpec keyForVersion(String version) {
        ...
        if (currentVersion.equals(version))
            return currentKey;
        if (previousKey != null && previousVersion.equals(version))
            return previousKey;
        throw new IllegalStateException(
    ...
    public static String decrypt(String stored) {
        ...
        try {
            ...
            log.error("PII decryption failed for a stored value (version {}) — wrong key, or corrupted data", version,
                    e);
