...
import com.patrick.fintech.loan_backend.service.MtnWebhookTransactionService;
...
public class PaymentWebhookController {
        ...
        public ResponseEntity<String> flutterwave(
                ...
                try {
                        ...
                        Long loanId = extractLoanIdFromReference(
                                        transactionReference);
                        ...
                        BigDecimal amount = extractAmount(
                                        data);
        ...
        public ResponseEntity<String> mtn(
                ...
                try {
                        ...
                        String mtnCurrency = firstText(root, "currency");
                        if (loan.getCurrency() != null && mtnCurrency != null &&
                                        !loan.getCurrency().equalsIgnoreCase(mtnCurrency.trim())) {
                                return ResponseEntity.badRequest()
                                                .body("Payment currency does not match loan currency");
                        }

                        boolean providerVerified = mtnMobileMoneyService.verify(transactionId);
                        if (!providerVerified) {
                        ...
                        PaymentGatewayResponse mtnResponse = mtnWebhookTransactionService.processWebhookConfirmation(
                                        ...
                                        amount.doubleValue(),
                                        mtnCurrency,
                                        "MTN_VERIFIED_WEBHOOK");

                        if (mtnResponse == null || !"SUCCESS".equalsIgnoreCase(mtnResponse.getStatus())) {
                                log.warn("[MTN WEBHOOK] Payment was not accepted. loanId={}, transactionId={}, status={}",
        ...
        public ResponseEntity<String> airtel(
                ...
                try {
                        ...
                        String airtelCurrency = firstText(root, "currency");
                        if (loan.getCurrency() != null && airtelCurrency != null &&
                                        !loan.getCurrency().equalsIgnoreCase(airtelCurrency.trim())) {
                                return ResponseEntity.badRequest()
                                                .body("Payment currency does not match loan currency");
                        }

                        if (!airtelMoneyService.verify(transactionId)) {
                                log.warn("[AIRTEL WEBHOOK] Provider verification failed. loanId={}, transactionId={}",
                                                loanId, transactionId);
        ...
        private boolean isValidSecret(
                ...
                byte[] actual = receivedSecret.trim().getBytes(StandardCharsets.UTF_8);
